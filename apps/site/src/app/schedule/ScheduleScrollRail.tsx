...
export const ScheduleScrollRail = ({
	getScrollable,
}: {
	getScrollable: () => HTMLDivElement | null;
}) => {
	...
	const getScrollMetrics = useCallback(() => {
		...
		const thumbHeight = Math.max(
			MIN_SCROLL_INDICATOR_HEIGHT_PX,
			(el.clientHeight / el.scrollHeight) * track.clientHeight,
		);
		return {
			maxIndicatorOffset: track.clientHeight - thumbHeight,
	...
	const handleThumbPointerDown = (e: PointerEvent<HTMLDivElement>) => {
		...
		dragStartRef.current = {
			pointerY: e.clientY,
			scrollTop: getScrollable()?.scrollTop ?? 0,
		};
	...
	const handleThumbPointerMove = (e: PointerEvent<HTMLDivElement>) => {
		...
		const delta =
			((e.clientY - dragStartRef.current.pointerY) /
				metrics.maxIndicatorOffset) *
			metrics.maxScrollTop;
		el.scrollTop = Math.min(
			metrics.maxScrollTop,
			Math.max(0, dragStartRef.current.scrollTop + delta),
		);
	};
	...
	return (
		<div
			className={styles.scheduleScrollRail}
			aria-hidden={!hasOverflowContent}
		>
			<button
				type="button"
				className={styles.scheduleScrollBtnUp}
				tabIndex={-1}
			...
			<button
				type="button"
				className={styles.scheduleScrollBtnDown}
				tabIndex={-1}
				disabled={!hasOverflowContent}
				onClick={() =>
					getScrollable()?.scrollBy({
